...
var mqttSystemName, systemName = "PowerMeter";// Define global system name for better MQTT tree definition
        ...
        if (err) {
            ...
            if (data.Configuration.MqttServer != null) {
                try {
					...
					console.log("Connecting to: " + data.Configuration.MqttServer);
                }
                catch (err) {
                    mqttClient = null;
                ...
                mqttClient = null;
				console.error("MqttServer is NULL: " + data.Configuration.MqttServer);
            }
    ...
    if (msg.Data != null) {
        ...
        if (circuit != null) {
            ...
            if (circuit.Samples.length > 0) {
                ...
                if (mqttClient != null) {
                    mqttSystemName = systemName + '/' + deviceName; // Define global system name for better MQTT tree definition
                    try {
                        mqttClient.publish(mqttSystemName + '/' + circuit.id + '/Name', circuit.Name);
                        mqttClient.publish(mqttSystemName + '/' + circuit.id + '/Voltage', circuit.Samples[0].vRms.round(1));
                        mqttClient.publish(mqttSystemName + '/' + circuit.id + '/Current', circuit.Samples[0].iRms.round(2));
                        mqttClient.publish(mqttSystemName + '/' + circuit.id + '/Watts', pTotal.round(1));
                        mqttClient.publish(mqttSystemName + '/' + circuit.id + '/Vars', qTotal.round(1));
                        mqttClient.publish(mqttSystemName + '/' + circuit.id + '/PowerFactor', circuit.Samples[0].pf.round(4));
                        mqttClient.publish(mqttSystemName + '/' + circuit.id + '/Timestamp', circuit.Samples[0].ts);
                        mqttClient.publish(mqttSystemName + '/' + circuit.id + '/Frequency', circuit.Samples[0].CalculatedFrequency.round(3));
                        if (circuit.LastDayKwh != null)
                            mqttClient.publish(mqttSystemName + '/' + circuit.id + '/LastDayKwh', circuit.LastDayKwh.round(1));
                    }
                    catch (err) {
